...
st.caption("v18.0 | 이름 기반 깔끔한 출력 & 기간 정밀 분석")
...
with col_s1:
    s_date = st.date_input("날짜 체크 시작 일시", datetime.date.today() - datetime.timedelta(days=7))
    s_time = st.time_input("시작 시간", datetime.time(0, 0))
with col_s2:
    e_date = st.date_input("날짜 체크 끝 일시", datetime.date.today())
...
if st.button("🚀 정밀 분석 시작"):
    ...
    with st.status("📡 인스티즈 서버 데이터 대조 중...", expanded=True) as status:
        try:
            ...
            # 결과 저장용 (이름만)
            names_to_check = [leader_name] + list(member_map.values())
            check_results = {name: False for name in names_to_check if name != "미지정"}
            ...
            for p_no in post_nos[:30]:
                ...
                if start_dt <= p_dt <= end_dt:
                    ...
                    for code, name in member_map.items():
                        # 식별값(번호표)이 소스코드에 있는지 확인
                        if code in combined_src:
                            check_results[name] = True
                            matched = True
                            break
                    
                    # 기간 내 글인데 멤버 번호표가 없다면? -> 방장(맹구) 글로 확정!
                    if not matched and leader_name in check_results:
